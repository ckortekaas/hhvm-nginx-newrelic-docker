# HHVM (3.7.0 / "Kanye West"), Nginx, New Relic on Ubuntu 14.04 Trusty via Phusion cleaned Ubuntu
...
RUN apt-get install -f -y --force-yes autoconf automake binutils-dev build-essential cmake g++ gawk git \
  ...
  php5 php5-dbg php5-common php5-mysql php5-mcrypt gperf libkrb5-dev \
    libc6-dev/trusty g++-4.8 libboost-context1.54-dev libboost1.54-dev libboost-filesystem1.54-dev \
    libboost-program-options1.54-dev libboost-regex1.54-dev libboost-system1.54-dev \
    libboost-thread1.54-dev libpam0g-dev libssl-dev libssl1.0.0/trusty libcurl3/trusty librtmp-dev libgd-dev \
    krb5-multidev libmagickcore-dev libsasl2-dev libkrb5-3 libk5crypto3 libgssapi-krb5-2 \
    libc6/trusty librsvg2-dev libgnutls-dev libkrb5-3/trusty libk5crypto3/trusty libgssapi-krb5-2/trusty \
    libgnutls26/trusty libgnutlsxx27/trusty libgnutls-openssl27/trusty libgcrypt11-dev/trusty \
    gir1.2-rsvg-2.0/trusty libglib2.0-dev/trusty libgdk-pixbuf2.0-dev/trusty libcairo2-dev/trusty \
    gir1.2-freedesktop/trusty libtasn1-6-dev/trusty
...
# Clone hhvm and switch to release 3.7 to get the hphpize tool needed for new relic hhvm ext
RUN cd /usr/local/src
RUN git clone https://github.com/facebook/hhvm.git --depth=1 /usr/local/src/hhvm
...
RUN cd /usr/local/src/hhvm; git checkout -b HHVM-3.7.0; rm -r third-party; git submodule update --init --recursive
RUN cd /usr/local/src/hhvm; cmake -DMYSQL_UNIX_SOCK_ADDR=/var/run/mysqld/mysqld.sock .
RUN cd /usr/local/src/hhvm; make -j 4; make install
